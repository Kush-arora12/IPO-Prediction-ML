...
axes[0].plot(fpr, tpr, color='darkorange', lw=2, label=f"AUC = {round(auc_score, 4)}")
axes[0].plot([0, 1], [0, 1], color='gray', linestyle='--', lw=1)
axes[0].set_title("ROC Curve - Gradient Boosting", fontsize=13)
...
axes[1].barh(feat_imp_sorted.index, feat_imp_sorted.values, color='darkorange', alpha=0.8)
axes[1].set_title("Feature Importance - Gradient Boosting", fontsize=13)
