...
# ==============================
# 1. FETCH & ENGINEER NIFTY FEATURES
# ==============================
...
def train_ipo_model(nifty, nifty_feature_cols):
    ...
    try:
        ...
        # Normalize dates and merge with Nifty
        ...
        print(f"Model trained on {len(X_train)} samples with {len(all_features)} features ✅")
...
def parse_date(text, ref_year):
    ...
    text = text.strip()
    for pat, fmt in [
        (r'([A-Z][a-z]{2})\s+(\d{1,2}),?\s*(\d{4})', 'mdy'),
        (r'(\d{1,2})[- ]([A-Z][a-z]{2})[- ](\d{4})',  'dmy'),
        (r'(\d{1,2})\s+([A-Z][a-z]{2})\s+(\d{4})',    'dmy'),
        (r'([A-Z][a-z]{2})\s+(\d{1,2})',               'md'),
        (r'(\d{1,2})\s+([A-Z][a-z]{2})',               'dm'),
    ]:
        m = re.search(pat, text)
        if m:
            g = m.groups()
            try:
                if fmt == 'mdy': return datetime(int(g[2]), month_map[g[0]], int(g[1])).date()
                elif fmt == 'dmy': return datetime(int(g[2]), month_map[g[1]], int(g[0])).date()
                elif fmt == 'md': return datetime(ref_year, month_map[g[0]], int(g[1])).date()
                elif fmt == 'dm': return datetime(ref_year, month_map[g[1]], int(g[0])).date()
            except: pass
    return None
...
# ==============================
# 4. Open IPOs from mainboard calendar
# ==============================
        ...
        for keyword, key in [("Opens on", "open"), ("Closes on", "close")]:
            if keyword in line:
                ...
                if m:
                    ...
                    d = parse_date(m.group(2), today.year)
                    if d:
                        ipo_events.setdefault(name, {})[key] = d

    open_ipos = []
    print(f"\n  Mainboard IPO events this month:")
    for name, dates in ipo_events.items():
        open_d, close_d = dates.get('open'), dates.get('close')
        status = "✅ OPEN TODAY" if (open_d and close_d and open_d <= today <= close_d) else "—"
...
def get_ipo_id(driver, ipo_name):
    ...
    print(f"    ❌ Could not find IPO ID for: {ipo_name}")
    return None, None


# ==============================
# 6. Scrape detail page
# ==============================
...
def run():
    # Step 1: Get Nifty data first
    nifty, nifty_feature_cols = get_nifty_features()

    # Step 2: Train model with Nifty features
    model, scaler, all_features = train_ipo_model(nifty, nifty_feature_cols)
    if not model:
        return

    # Step 3: Get latest Nifty snapshot for live prediction
    ...
    if not open_ipos:
        print("\nNo open mainboard IPO detected today ❌")
        driver.quit()
        return

    print(f"\n→ Found {len(open_ipos)} open IPO(s). Scraping live data...\n")
    print("─" * 60)
    ...
    for ipo in open_ipos:
        ...
        if data:
            live_input = pd.DataFrame([{
                ...
                **latest_nifty.to_dict()   # Inject live Nifty features
            }])
        else:
            # Fallback with neutral values + live Nifty
            live_input = pd.DataFrame([{
                'Issue_Size(crores)': 1000,
                'QIB': 1, 'HNI': 1, 'RII': 1,
                'Total': 2,
                'Offer Price': 300,
        ...
        results.append({**ipo, "prob": prob_pct, "rec": rec, "data": data, "warn": warn})
