...
def train_ipo_model(nifty, nifty_feature_cols):
    ...
    try:
        ...
        print(f"  Best K: {best_k}  (Accuracy: {round(max(k_scores)*100, 2)}%)")
        ...
        return model, scaler, all_features, X_train_scaled, y_train
...
def run():
    ...
    model, scaler, all_features, X_train_scaled, y_train = train_ipo_model(nifty, nifty_feature_cols)
    if not model:
        return
    ...
    for ipo in open_ipos:
        ...
        # Skip if slug not found — IPO not active yet
        if not data:
        ...
        distances, indices = model.kneighbors(live_scaled)
    ...
    for r in results:
        ...
        if r.get('warn'):
            print(f"     {r['warn']}")
        print(f"     Nearest Neighbor Votes:")
        for i, (label, dist) in enumerate(zip(r['neighbor_labels'], r['neighbor_dists'])):
            print(f"       #{i+1}  {'Gain ✓' if label == 1 else 'No Gain ✗'}  "
